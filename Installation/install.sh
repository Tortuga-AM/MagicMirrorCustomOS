#!/bin/bash
# This is the installation script that should pull the CustomMagicMirrorOS image and install it.
# It is assumed that the user has already setup raspberry pi os and has an internet connection.
# The script will install the CustomMagicMirrorOS image and configure it to boot to the MagicMirror.
# The script will also install the necessary dependencies and configure the display settings.
...
if [ $USER_ID != "AM2024ZZmm2os" ]; then
        echo "Failed user AUTH..."
        exit 2
fi
echo "User AUTH successful..."

read -p "Has this device been updated(y/n)?  " updated
if [ "$updated" != "y" ]; then
        echo "Please Update Your Device First..."
        exit 3
fi
...
if [ $doInstall == 1 ]; then
	...
	if git clone --depth=1 https://github.com/MagicMirrorOrg/MagicMirror.git; then
		...
		curl -sL https://raw.githubusercontent.com/Tortuga-AM/MagicMirrorCustomOS/refs/heads/main/UpdateHandler/simple-update.sh >MagicMirror/simple-update.sh
		chmod +x MagicMirror/simple-update.sh
